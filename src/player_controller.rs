...
fn toggle_grab_cursor(window: &mut Window) {
	...
	window.cursor.grab_mode = if window.cursor.visible { CursorGrabMode::None } else { CursorGrabMode::Locked };
}

fn initial_grab_cursor(mut window_query: Query<&mut Window>) {
   	let Ok(mut primary) = window_query.get_single_mut() else { return; };	   
...
fn handle_mouse_input(
	...
	if primary.cursor.grab_mode == CursorGrabMode::Locked {
		for (mut transform, mut player_controller) in query.iter_mut() {
			if let Some(rotation) = mouse_motion.read().map(|m| m.delta)
				.reduce(|a, e| a + e) {
				let mut new_pitch =
